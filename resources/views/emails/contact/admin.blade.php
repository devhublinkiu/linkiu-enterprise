...
<head>
    ...
    <style>
        ...
        .field { margin-bottom: 15px; padding: 10px; background: #fdfdfd; border: 1px solid #f0f0f0; border-radius: 5px; }
        .label { font-weight: bold; color: #555; display: block; margin-bottom: 2px; font-size: 12px; text-transform: uppercase; }
        .value { color: #000; font-size: 15px; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <div class="field">
                <span class="label">Empresa / NIT:</span>
                <span class="value">{{ $submission->company_name ?? 'N/A' }} ({{ $submission->nit ?? 'N/A' }})</span>
            </div>
            ...
            <div class="field">
                ...
                <span class="value" style="color: #DD301B; font-weight: bold;">{{ $submission->service }}</span>
            </div>

            <div class="field">
                <span class="label">Tipo de Consulta:</span>
                <span class="value">{{ is_array($submission->types) ? implode(', ', $submission->types) : $submission->types }}</span>
            </div>

            <div class="field">
                <span class="label">Mensaje:</span>
                <p class="value" style="font-style: italic;">"{{ $submission->message }}"</p>
            </div>
            
            <p style="font-size: 11px; color: #999;">IP del remitente: {{ $submission->ip_address }}</p>
        </div>
