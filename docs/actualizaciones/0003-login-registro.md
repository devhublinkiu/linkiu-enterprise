...
- **Estado:** Planificado — listo para `#go` por corte (empezando por **3A · Input OTP**)
- **Fecha:** 2026-09-12 (ampliado y planificado 2026-09-13)
...
| **Input OTP** | **código de 6 dígitos (registro + recuperación)** | 🔴 **NUEVO — construir primero (Corte 3A)** |

- **Input OTP** es el único componente que falta. Es de Fase 2 (`orden-componentes.md`) y requiere el
  paquete **`input-otp`** (lib de @guilhermerodz, como Command necesita `cmdk`). Se instala en 3A.
...
### Corte 3E · Recuperación de contraseña con OTP (backend + UI)
...
**Decisiones menores heredadas de Corte 2 (aún abiertas):** redirección del superadmin
(`admin.associates.index` por ahora) y `WelcomeUser` síncrono vs `defer()`.

## §8 Aprobación

- [x] Decisiones §7 resueltas
- [ ] Plan revisado (`#plan`/`#review`)
- [ ] `#go` por corte (empezando por **3A · Input OTP**)

---

## Deuda pre-existente (ajena a estos cortes)

13 tests de Breeze fallan porque prueban rutas que el proyecto no implementaba (recuperar/confirmar
contraseña, verificación de correo) o por `service_categories` en la home (`ExampleTest`). Los de
recuperación **dejarán de ser obsoletos** cuando se haga 3E (habrá que adaptarlos al flujo OTP, no
al de token). El resto se limpia en 3G.
