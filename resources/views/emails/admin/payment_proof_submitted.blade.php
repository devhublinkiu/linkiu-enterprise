...
<body>
    <div class="container">
        ...
        <div class="content">
            <p>Administrador,</p>
            <p>El socio <strong>{{ $paymentRequest->associate->company_name }}</strong> ha subido un nuevo comprobante de pago para su revisión.</p>
            
            <p><strong>Monto Reportado:</strong> ${{ number_format($paymentRequest->amount, 2) }}</p>
            <p><strong>Referencia:</strong> {{ $paymentRequest->reference_number }}</p>
            
            <p>Por favor, ingrese al panel para verificar la transacción en la cuenta bancaria y proceder con la aprobación.</p>
            
            <div style="text-align: center;">
                <a href="{{ url('/admin/payment-requests/' . $paymentRequest->id) }}" class="btn">Revisar Pago</a>
            </div>
        </div>
